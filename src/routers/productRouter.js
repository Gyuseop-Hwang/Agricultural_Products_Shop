...
productRouter.put(
  ...
  wrapAsync(async (req, res) => {
    ...
    const product = await productService.updateComment(productId, commentId, req.body);

    res.status(201).json(product)
  }))

productRouter.delete('/products/:productId/comments/:commentId', loginRequired, wrapAsync(async (req, res) => {

  const { productId, commentId } = req.params;
  await productService.deleteComment(productId, commentId);

  res.status(200).json({ result: "success", message: "리뷰가 성공적으로 삭제되었습니다." })
}))
