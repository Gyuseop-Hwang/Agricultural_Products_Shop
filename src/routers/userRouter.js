...
import { BadRequestError, ForbiddenError, wrapAsync } from '../utils';
...
userRouter.patch('/users/userInfo', loginRequired, wrapAsync(async (req, res) => {
  ...
  if (!currentPassword) {
    throw new ForbiddenError('정보를 변경하려면, 현재의 비밀번호가 필요합니다.');
  }
...
userRouter.get('/admin/userlist', loginRequired, authRequired, wrapAsync(async (req, res) => {
